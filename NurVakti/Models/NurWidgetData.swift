...
extension NurWidgetData {
    ...
    /// Ana uygulama tarafından çağrılır — verileri widget'a yazar
    public static func save(_ data: NurWidgetData) {
    ...
    public func dataForDate(_ date: Date) -> NurWidgetData {
        ...
        if let matchingDay = self.days?.first(where: { cal.isDate($0.date, inSameDayAs: date) }) {
            ...
            // Gün eşleşmediyse mevcut allPrayers listesini hedeflenen güne göre ayarla
    ...
    /// Sadece zikir verisini günceller
    public static func updateDhikr(name: String, count: Int, target: Int) {
    ...
    /// Vakit verilerini (çok günlük dahil) günceller
    public static func updatePrayers(
        nextName: String,
        nextNameEn: String,
        nextTime: Date,
        all: [WidgetPrayerEntry],
        days: [WidgetDayPrayerData]? = nil,
        city: String,
        hijri: String,
        lang: String
